...
export function analysePng(buf) {
  ...
  return {
    ...
    // Every opaque pixel is pure white (the "white logo for dark backgrounds"
    // brand variant): bleed/needsBacking never see this since it's neither dark
    // nor sparse, and it renders as a blank square on the app's white tile.
    whiteOnly: meanLum > 0.9 && lumRange < 0.05,
  };
}
