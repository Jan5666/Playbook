...
- `buildFetchPlan({ fastTiers, lazyLists, warmed, activeView }) → { order, key }` where
  ...
  - `key` = the **fast-tier membership signature**: the positions/watchlist/alerts/ribbon price-keys only, **sorted**, joined. Order-independent, and deliberately *excludes* the warmed lazy lists — so warming a lazy list does not change `key` (no redundant auto-fetch); lazy fetches are driven by the view effect, and the regular interval keeps warmed lists fresh because `order` includes them.
...
- **Node unit tests** for the pure kernel (`backend/test/fetch-plan.test.mjs`, new): float-active-to-front ordering; fast-tier-first when no lazy view active; dedupe preserves first occurrence; **`key` is identical under reorder AND under lazy-list warming, but differs when fast-tier membership changes** (the anti-thrash invariant); warmed-but-inactive lists included in `order` after the fast tier. Plus an anti-drift guard: `app.js` binds `buildFetchPlan` from `PBCore` and defines no local copy.
